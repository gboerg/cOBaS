...
class Frame(ctk.CTk):
    ...
    def createBackground(self):
        background = self.background = ctk.CTkFrame(master=self, width=500, height=200, fg_color="black")
    ...
    def createObsFrame(self):
        connectFrame = self.connectFrame = ctk.CTkFrame(master=self.background, fg_color="blue")
        ...
        host = self.host_entry = ctk.CTkEntry(master=self.connectFrame, placeholder_text="IP-Adress of target PC")
        ...
        port = self.port_entry = ctk.CTkEntry(master=self.connectFrame, placeholder_text="WebSocket Port")
        ...
        password = self.password_entry = ctk.CTkEntry(master=self.connectFrame, placeholder_text="WebSocket Password", show="*") # NOTE: Added show="*" for password security
        self.password_entry.grid(row=0, column=5, padx=10, pady=10)



        return [connectFrame, host, port, password]

    


    def createInnerFrame(self):
        innerFrame = self.inner = ctk.CTkFrame(master=self.background, fg_color="red")
        innerGrid = self.inner.grid(row=1, column=0, sticky="nsew")
        return innerFrame
